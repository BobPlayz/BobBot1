...
import type { Bank } from "../schema/bank.js";
...
export async function upgradeBank(
  discordId: string,
): Promise<
  | {
      success: true;
...
> {
  ...
  const cost = bank.bankLevel * 5000;
  ...
  await db
    .update(bankTable)
    .set({
      bankLevel: bank.bankLevel + 1,
      maxStorage: bank.maxStorage + 5000,
      interestRate: bank.interestRate + 1,
    })
    .where(eq(bankTable.discordId, discordId));

  return {
    success: true,
    newLevel: bank.bankLevel + 1,
    newStorage: bank.maxStorage + 5000,
  };
}
...
export async function getUserInventory(
  ...
  return db
    ...
    .innerJoin(
      itemsTable,
      eq(inventoryTable.itemId, itemsTable.id),
    )
