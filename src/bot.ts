...
import { handleActionCommand, handleActionMessage } from "./commands/actions.js";
import type { ActionType } from "./utils/nekos.js";
import { handleAnnounce } from "./commands/announce.js";
import { handleGiveawayCommand, handleGiveawayEnter } from "./commands/giveaway.js";
import {
  handleBalance, handleWork, handleFish, handleHunt, handleShop, handleBuy,
  handleInventory, handleBuyAutocomplete, handleDaily, handleLeaderboard,
  handleRob, handleCoinflip, handleDice,
...
export async function startBot(): Promise<void> {
  ...
  const client = new Client({
    intents: [
      ...
      GatewayIntentBits.MessageContent, // Enable in Discord Dev Portal → Bot → Privileged Intents
    ],
  ...
  // ── Prefix commands: "bob slap @user", "bob bite @user", etc. ─────────────
  client.on(Events.MessageCreate, async (message: Message) => {
    if (message.author.bot) return;
    const content = message.content.trim().toLowerCase();
    if (!content.startsWith(PREFIX + " ")) return;
    const sub = content.slice(PREFIX.length).trim().split(/\s+/)[0] as ActionType;
    if ((ACTION_COMMANDS as string[]).includes(sub)) {
      await handleActionMessage(message, sub).catch((err) => {
        logger.error({ err }, "Prefix command error");
  ...
  client.on(Events.InteractionCreate, async (interaction: Interaction) => {
    try {
      if (interaction.isAutocomplete()) {
        if (interaction.commandName === "buy") await handleBuyAutocomplete(interaction);
        return;
      }

      if (interaction.isButton()) {
        if (interaction.customId.startsWith("ttt_"))       await handleTTButton(interaction);
  ...
  await handleMarriageButton(interaction);
      ...
      if ((ACTION_COMMANDS as string[]).includes(commandName)) {
        await handleActionCommand(interaction, commandName as ActionType); return;
      }

      switch (commandName) {
        case "announce":    await handleAnnounce(interaction);      break;
        case "giveaway":    await handleGiveawayCommand(interaction); break;
        ...
        case "fish":        await handleFish(interaction);          break;
        case "hunt":        await handleHunt(interaction);          break;
        ...
        case "connectfour": await handleConnectFour(interaction);   break;
        case "8ball":       await handleEightBall(interaction);     break;
